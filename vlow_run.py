...
if __name__ == '__main__':
    ...
    assert args.method in ['tabsynvlow','contvae','catvae','poissonvae','vqvae'], f'invalid method: {args.method}'
    
    ## running the model
    curr_dir = os.path.dirname(os.path.abspath(__file__))
    match args.vae_prior:
        case 'normal': vae_source_folder = 'contvae'
        case 'laplace': vae_source_folder = 'contvae'
        case 'categorical': vae_source_folder = 'catvae'
        case 'poisson': vae_source_folder = 'poissonvae'
        case 'vqvae': vae_source_folder = 'vqvae'
        
    prior_folder = f'/{args.vae_prior}' if args.vae_prior in ['normal','laplace'] else ''

    if args.train:
        if args.method=='tabsynvlow':
            if not os.path.exists(f'{curr_dir}/tabsynvlow/{vae_source_folder}/ckpt{prior_folder}/{dataname}/train_z.npy'):
                run_command(f"python main.py --dataname {dataname} --method {vae_source_folder} --vae_prior {args.vae_prior} --mode train", env_name="tabsyn")
            else:
                print(f'vae using {args.vae_prior} prior has been trained already')
            
        run_command(f"python main.py --dataname {dataname} --method {args.method} --vae_prior {args.vae_prior} --mode train --cond_vel {args.cond_vel}", env_name="tabsyn")
