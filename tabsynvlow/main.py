...
def main(args): 
    ...
    model = ContVarFlowMatching(args.var_weight)
    ...
    net = Net(in_dim, 512, learnable=learnable).to(device)
    # net = Net(in_dim, 512).to(device) ## already according to tabsyn paper
    ...
    print(f'training using {args.cond_vel} conditional velocity formula and {args.var_weight} weight')
    # net = Net(in_dim, in_dim, [1024, 2048, 2048, 1024], 10).to(device)
    v_t = CondVF(net, net_t)
    ...
    for epoch in range(num_epochs):
        ...
        if curr_loss < best_loss:
            best_loss = curr_loss
            patience = 0
            torch.save(v_t.net.state_dict(), f'{ckpt_path}/model{var_caption}_{args.cond_vel}.pt')
        else:
            patience += 1
            # if patience == 500:
        ...
        if epoch % 1000 == 0:
            if not os.path.exists(f'{ckpt_path}/per_epoch'):
                os.makedirs(f'{ckpt_path}/per_epoch')
            torch.save(v_t.net.state_dict(), f'{ckpt_path}/per_epoch/model{var_caption}_{args.cond_vel}_{epoch}.pt')
